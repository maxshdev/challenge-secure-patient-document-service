###############################################################################
# Secure Patient Document Service — Main Terraform Configuration
#
# This is the root module that composes all infrastructure components.
# It provisions a complete AWS environment for the patient document service:
#   - VPC with public/private subnets
#   - ECS Fargate cluster behind an ALB
#   - RDS PostgreSQL (encrypted, private)
#   - S3 bucket (SSE-KMS, no public access)
...
terraform {
  ...
  # backend "s3" {
  #   bucket         = "patient-docs-terraform-state"
...
provider "aws" {
  ...
  default_tags {
    tags = {
      Project     = "patient-document-service"
...
# RDS: Allow connections only from ECS tasks
...
resource "aws_kms_key" "main" {
  description             = "KMS key for ${var.project_name} — encrypts RDS, S3, and Secrets Manager"
...
# Enable versioning (for compliance — prevent accidental deletion)
...
# Bucket policy — only allow ECS task role
...
# ECS task role (application-level permissions — least privilege)
...
# S3 permissions — only the specific bucket, specific actions
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([{
    ...
    healthCheck = {
      command     = ["CMD-SHELL", "curl -f http://localhost:${var.app_port}/api || exit 1"]
      interval    = 30
      timeout     = 5
      retries     = 3
      startPeriod = 60
